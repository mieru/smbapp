package utils.faktpdfgenerator;
...
@Stateless
@LocalBean
public class FaktPdfGenerator { 
	private DecimalFormat df = new DecimalFormat("#.##");
	
	public String generatePdf(FakturaSprzedazy fakturaSprzedazy, Float wartoscAllNetto, Float wartoscAllBrutto) throws AddressException, MessagingException, DocumentException,
			SQLException, IOException {
		JSONArray jsonArray = new JSONArray(fakturaSprzedazy.getListaTowarow());
		Document document = new Document();
		String filePath = "\\faktury\\FakturaVAT" + fakturaSprzedazy.getIdFaktury() + ".pdf";
		...
		headerTable.addCell(getCell("\nSprzedawca:\n" + fakturaSprzedazy.getDaneWystawiajacego() + "\n", PdfPCell.ALIGN_LEFT));
		headerTable.addCell(getCell("Faktura VAT nr " + fakturaSprzedazy.getNumerFaktury() + "\n", PdfPCell.ALIGN_CENTER));
		headerTable.addCell(getCell("\nNabywca:\n" + fakturaSprzedazy.getDaneKlienta() + "\n", PdfPCell.ALIGN_RIGHT));
		document.add(headerTable);
		SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd");
		document.add(new Paragraph("Data wystawienia: " + simpleDateFormat.format(new Date(fakturaSprzedazy.getDataWystawienia().getTime()))));
